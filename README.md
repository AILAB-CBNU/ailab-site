...
2026-09-23 누적 업데이트: [설치·과제/갤러리 관리·홈 스크롤 조절 안내](docs/lab-update-20260923.md). 관리자에서 연구 과제의 연도/진행 상태와 갤러리 사진을 관리하며, 등록 데이터와 업로드 사진은 서버 `seminar-data/`에만 저장합니다. ZIP 생성: `python3 scripts/build_lab_addon.py`.
...
매일 오전 9시 라운지 요약·개인 할 일 알림은 [아침 브리핑 설정 안내](docs/lab-briefing.md)를 참고하세요. `14-SETUP-LAB-BRIEFING.bat`로 설정하고 기존 `10`번 봇에서 실행합니다. OpenAI 키와 업무 기록은 서버에만 저장합니다.
